  ...
  public int calculateTestResult(final List<Answer> studentAnswers) {
    ...
    for (Question question : questions) {
      ...
      int correctAnswersAmount =
          question.getAnswers().stream().filter(Answer::isCorrect).collect(Collectors.toList()).size();
      ...
      if (question.getType() == QuestionType.Gap) {
        for (Answer gapAnswer : question.getAnswers()) {
          Answer studentGapAnswer =
              studentAnswers.stream().filter(answer -> answer.equals(gapAnswer)).collect(Collectors.toList()).get(0);
          // Correct Answer
