...
public class SeminarService {
  ...
  public List<Seminar> allWithTestsStartingToday() {
    ...
    List<Seminar> seminarsWithTest =
        allSeminars.stream().filter(seminar -> seminar.getTest() != null).collect(Collectors.toList());
    return seminarsWithTest.stream().filter(seminar -> seminar.getTest().getBeginDate().isEqual(LocalDate.now()))
