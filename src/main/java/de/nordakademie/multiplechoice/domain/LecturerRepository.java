...
public class LecturerRepository {
  ...
  public Lecturer byId(final long lecturerId) {
    try {
      return entityManager.createQuery("Select lecturer FROM Lecturer lecturer WHERE userId = :id", Lecturer.class)
                          .setParameter("id", lecturerId).getSingleResult();
  ...
  public Lecturer byMail(final String mail) {
    try {
      return entityManager.createQuery("SELECT lecturer FROM Lecturer lecturer WHERE email = :mail", Lecturer.class)
